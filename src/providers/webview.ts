...
export class RadarWebview {
  private panel: vscode.WebviewPanel | undefined;
  private lastRisks: DependencyRisk[] = [];

  constructor(private readonly context: vscode.ExtensionContext) {}

  show(risks: DependencyRisk[] | null = null) {
    if (risks) {
    ...
    if (!this.panel) {
      this.panel = vscode.window.createWebviewPanel(
        "rnDependencyRadar.details",
        "RN Dependency Radar — Dependencies",
  ...
  private renderHtml(risks: DependencyRisk[]): string {
    const runtimeRisks = risks.filter(
      (r) => !r.dependency.meta.isDevDependency
    );
    ...
    const byRiskRuntime = runtimeRisks.reduce(
      (acc, r) => {
    ...
    const byRiskDev = devRisks.reduce(
      (acc, r) => {
        acc[r.riskLevel] = (acc[r.riskLevel] ?? 0) + 1;
        return acc;
      },
    ...
    const byTypeRuntime = runtimeRisks.reduce(
      (acc, r) => {
    ...
    const byTypeDev = devRisks.reduce(
      (acc, r) => {
        const t = r.dependency.type;
        acc[t] = (acc[t] ?? 0) + 1;
        return acc;
    ...
    const versionBucketsRuntime = runtimeRisks.reduce(
      (acc, r) => {
    ...
    const versionBucketsDev = devRisks.reduce(
      (acc, r) => {
        const installed = r.dependency.version;
        const latest = r.dependency.meta.latestVersion;
        if (!latest) {
    ...
    const pctRuntime = (count: number) =>
      totalRuntime > 0 ? Math.round((count / totalRuntime) * 100) : 0;
    const pctDev = (count: number) =>
    ...
    const renderRows = (list: DependencyRisk[]) =>
      list.map((r) => {
        ...
        const hasNewerVersion =
          latestNormalized !== null && installedNormalized !== latestNormalized;
        ...
        const rulesText =
          r.triggeredRules.map((rr) => `• ${rr.message}`).join("\n") ||
        ...
        const riskToClass = (risk: string) =>
          risk === "high" ? "badge-high" : risk === "medium" ? "badge-medium" : "badge-low";
        ...
        const versionDiff =
          d.meta.missedVersionsCount != null && d.meta.missedVersionsCount > 0
