...
class RulesEngine {
    constructor(staticRules, thresholds) {
    ...
    evaluate(dependency, project, scorer) {
        ...
        const libraryRisk = libraryRules.reduce((acc, r) => scorer.maxRisk(acc, r.risk), "low") || "low";
        const projectRisk = projectRules.reduce((acc, r) => scorer.maxRisk(acc, r.risk), "low") || "low";
        const highestRisk = scorer.maxRisk(libraryRisk, projectRisk);
        const justifications = highestRisk === "low"
    ...
    matches(rule, dep, project) {
        ...
        const matchesNumeric = (expr, value) => {
            if (value == null) {
                return false;
            }
            const op = expr[0];
        ...
        if (c.installedLastUpdateMonths &&
